...
impl Conn {
    pub fn new(socket: TcpStream, addr: impl Into<SocketAddr>) -> Self {
        ...
        Self {
            socket,
            addr: addr.into(),
            buffer: BytesMut::with_capacity(BUFF_SIZE), // TODO: better value
        }
    }
    ...
    pub async fn read_frame(&mut self) -> crate::Result<Option<Frame>> {
        loop {
            ...
            // of stream".
            self.buffer.resize(BUFF_SIZE, 0);
            let cnt = self.socket.read(&mut self.buffer).await?;
            self.buffer.truncate(cnt);
            if 0 == cnt {
                ...
                if self.buffer.is_empty() {
                    ...
                    return Err(Error::Internal("connection reset by peer".to_string()));
                }
            }
        }
    }
    ...
    pub async fn write_frame(&mut self, frame: &Frame) -> io::Result<()> {
        ...
        match frame {
            Frame::Array(val) => {
                ...
                self.socket.write(&[b'*' as u8]).await?;
